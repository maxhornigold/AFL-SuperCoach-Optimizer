...
from helper import print_optimal_solution, print_optimal_scores
...
b_r = {r: m.addVar(vtype=GRB.CONTINUOUS, lb=0) for r in R}
...
constr_4_1_7 = m.addConstr(
    b_r[0] + quicksum(quicksum(v[p,0]*x[p,q,0] for q in Q_p[p]) for p in P) == B
    )

# record the budget for the next round.
constr_4_1_8 = {r: m.addConstr(
    b_r[r] == (b_r[r-1] + 
...
print_optimal_solution(m, P, P_q, R[0:1], Q_score, Q_sub, x, x_bar, t_in, 
                       t_out, b_r, c, psi)

# print scores each week. Change R as desired.
# print_optimal_scores(m, P, P_q, R, Q_score, Q_sub, x, x_bar, t_in, 
#                      t_out, b_r, c, psi)